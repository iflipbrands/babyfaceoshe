...
## Admin JSON import + docked player playlist

- **Import JSON:** Admin now has an **IMPORT JSON** button. Choose a previously exported `babyface-oshe-content.json` file. It also accepts a valid `content.js` file from this site.
- **Download JSON:** Use **DOWNLOAD JSON** when you want a portable backup you can import later.
- **Load in Bottom Player:** Each music slot has a checkbox. Check any songs you want loaded into the docked player playlist. The **first checked song is preloaded when the page opens**, so the bottom play button works immediately after the visitor taps it—without first scrolling to Music.
- You may check multiple songs. When a checked song finishes, the player advances to the next checked song.
- If an older content file has no player checkboxes saved yet, the site falls back to the first song that has an audio path so the player does not appear dead.


## Mobile fix notes
- Current content from babyface-oshe-content.json is baked into assets/js/content.js so mobile devices receive the same music data as desktop.
- Audio and cover URLs use local assets paths for reliable GitHub Pages playback.
- Admin player checkboxes are compact.
- Mobile hero height is tightened to remove the large gap before Tour Dates.
